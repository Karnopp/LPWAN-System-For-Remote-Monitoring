...
$query = "SELECT * FROM nivel_agua ORDER BY dia DESC, horario DESC";
...
echo "<tr><th class='coluna' >Status do Nível</th>";
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['Data', 'Nível'],
          <?php
          $query = "SELECT * FROM nivel_agua ORDER BY dia ASC, horario ASC";
          $result = mysqli_query($link, $query);
          while ($row = mysqli_fetch_row($result)) {
            if($row[1]=="ALTO"){
              $temp=1;
            ...
            $data = explode("-", $row[3]);
            $tempo = explode(":", $row[4]);
            echo "[new Date(".$data[0].",".(string)((int)$data[1]-1).",".$data[2].",".$tempo[0].",".$tempo[1].",".$tempo[2]."),".$temp."],";
          }
          mysqli_close($link);
          ?>
        ]);
